 ...
 *
 * Vendored from the PrismaUI SKSE example plugin (PrismaUI-SKSE/example-skse-plugin,
 * src/PrismaUI_API.h, framework v1.4.x). Only used to talk to the PrismaUI.dll when it
 * is present at runtime — Isekai Hero has NO build- or load-time dependency on PrismaUI.
 */
#pragma once
